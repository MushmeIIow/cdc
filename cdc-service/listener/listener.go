...
import (
	...
	"github.com/mushmellow/cdc-service/protobuf"
	...
	for {
		...
		if msg != nil {
			if msg.WalMessage != nil {
				...
				LogByteaProtoFields(tx.Actions)
...
// LogByteaProtoFields log bytea proto fields
func LogByteaProtoFields(actions []ActionData) {
	logrus.Debugln("parse data:", actions)

	for _, action := range actions {
		if action.Table != "protobuf" {
			continue
		}

		for _, column := range action.Columns {
			if column.valueType != pgtype.ByteaOID {
				continue
			}

			logrus.Infoln("proto field:", column.value)

			reqBytes, err := hex.DecodeString(string(column.value.(string)))
			if err != nil {
				log.Fatalln("Failed to bytes:", err)
			}

			req := &protobuf.HelloRequest{}
			if err := proto.Unmarshal(reqBytes, req); err != nil {
				log.Fatalln("Failed to parse proto:", err)
			}

			logrus.Infoln("after unmarshal proto field:", req)
		}
	}
}
